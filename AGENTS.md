...
**Job registry (required):** Every periodic job MUST be registered in `memory/cron-registry.json` (id, category: reminder/report/automation, purpose, schedule, deliveryTarget, reviewBy, agentTurn, runner). Decide `agentTurn` first: if the job is a pure relay (deterministic script stdout → chat, no LLM judgment), set agentTurn=false and run it via the OS scheduler (`scripts/relay_run.py <id>` + a launchd/systemd unit) — do NOT create an LLM cron for it. Only jobs needing LLM judgment/generation (agentTurn=true) use the LLM cron. Update the registry in the same turn you add/edit/remove a job — never leave them out of sync. `scripts/cron_registry_check.py` detects drift and reports it.
